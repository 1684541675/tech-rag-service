...
def create_agent_app(*, make_graph: GraphFactory) -> FastAPI:
    ...
    @app.post("/agent/run")
    def run(request: AgentRunRequest) -> dict[str, object]:
        run_id = uuid4().hex
        ...
        if "__interrupt__" in result:
            ...
            return {"status": "awaiting_approval", "run_id": run_id, "thread_id": thread_id, "interrupt": result["__interrupt__"][0].value}
        return _public_result(result, run_id=run_id, thread_id=thread_id)

    @app.post("/agent/{thread_id}/approval")
    def approval(thread_id: str, request: ApprovalRequest) -> dict[str, object]:
        ...
        return _public_result(result, run_id=str(result.get("run_id", "")), thread_id=thread_id)

    return app


def _public_result(result: dict[str, object], *, run_id: str, thread_id: str) -> dict[str, object]:
    return {
